...
const p_dateTemplate2 = (rowData, { rowIndex }) => <p >{(new Date(rowData.nextservicedate)).toLocaleDateString()}</p>
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.notes}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "maintenanceschedules"});
...
<Column field="nextservicedate" header="Nextservicedate" body={p_dateTemplate2} filter={selectedFilterFields.includes("nextservicedate")} hidden={selectedHideFields?.includes("nextservicedate")}  sortable style={{ minWidth: "8rem" }} />
