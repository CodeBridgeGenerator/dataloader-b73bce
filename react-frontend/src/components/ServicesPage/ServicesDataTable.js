...
const p_dateTemplate2 = (rowData, { rowIndex }) => <p >{(new Date(rowData.price)).toLocaleDateString()}</p>
const p_numberTemplate3 = (rowData, { rowIndex }) => <p >{rowData.duration}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "services"});
...
<Column field="price" header="Price" body={p_dateTemplate2} filter={selectedFilterFields.includes("price")} hidden={selectedHideFields?.includes("price")}  sortable style={{ minWidth: "8rem" }} />
